(ns com.juhezi.sicp.weibo-str
  (:import (com.juhezi.sicp.matrix)))
...
(defn convert-2D-array [content]
  ...
  (first (reduce (fn [[array index] item]
                   (let [new-item (convert item)
                     ...
                     [(com.juhezi.sicp.matrix/update-value array current-line current-row new-item) (inc index)]))
                 [(com.juhezi.sicp.matrix/build-2d-array (get-line-count content) max-length) 0]
                 (seq content))))

(defn weibo-str [content]
  (clojure.string/join "\n"
                       (map #(clojure.string/join separator %)
                            (com.juhezi.sicp.matrix/cols (convert-2D-array content)))))